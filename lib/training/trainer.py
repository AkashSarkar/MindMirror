    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            if verbose and (epoch + 1) % eval_every == 0:
                ...
                print(f"Epoch {epoch + 1}/{epochs} | " f"Train Loss: {train_loss:.4f}")
...
def split_data(
    data: List[Any], train_ratio: float = 0.8, val_ratio: float = 0.1
...
def plot_training_history(
    ...
    try:
        ...
        if "perplexity" in history:
            axes[1].plot(
                val_epochs, history["perplexity"], label="Perplexity", alpha=0.7
            )
