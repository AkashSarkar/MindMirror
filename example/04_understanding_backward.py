...
def demonstrate_backward_step_by_step():
    ...
    print(
        f"   1. w1 * x1 = {neuron.w[0].data:.3f} * {x[0].data} = {w1_times_x1.data:.3f}"
    )

    w2_times_x2 = neuron.w[1] * x[1]
    print(
        f"   2. w2 * x2 = {neuron.w[1].data:.3f} * {x[1].data} = {w2_times_x2.data:.3f}"
    )

    sum_weighted = w1_times_x1 + w2_times_x2 + neuron.b
    print(
        f"   3. sum + bias = {w1_times_x1.data:.3f} + {w2_times_x2.data:.3f} + {neuron.b.data:.3f} = {sum_weighted.data:.3f}"
    )
    ...
    print(
        f"   Weight 1: {'↗️ Increase' if neuron.w[0].grad < 0 else '↘️ Decrease'} to reduce loss"
    )
    print(
        f"   Weight 2: {'↗️ Increase' if neuron.w[1].grad < 0 else '↘️ Decrease'} to reduce loss"
    )
    print(
        f"   Bias: {'↗️ Increase' if neuron.b.grad < 0 else '↘️ Decrease'} to reduce loss"
    )
    ...
    print(
        f"   New Weight 1: {old_w1:.3f} - {learning_rate} * {neuron.w[0].grad:.3f} = {new_w1:.3f}"
    )
    print(
        f"   New Weight 2: {old_w2:.3f} - {learning_rate} * {neuron.w[1].grad:.3f} = {new_w2:.3f}"
    )
    print(
        f"   New Bias: {old_b:.3f} - {learning_rate} * {neuron.b.grad:.3f} = {new_b:.3f}"
    )
    ...
    print(
        f"   Loss improvement: {loss.data - new_loss.data:.6f} {'✅' if new_loss.data < loss.data else '❌'}"
    )
