...
def generate_text(
    network, chars, char_to_idx, idx_to_char, sequence_length, seed_text, length=50
):
    """
    ...
    for _ in range(length):
        ...
        try:
            ...
            input_indices = [
                char_to_idx.get(ch, char_to_idx[" "]) for ch in current_sequence
            ]
...
if __name__ == "__main__":
    # Step 1: Prepare data
    text_data, chars, char_to_idx, idx_to_char, X, Y, sequence_length = (
        build_character_level_text_generator()
    )

    # Step 2: Train network
    network = create_and_train_network(
        X, Y, chars, char_to_idx, idx_to_char, sequence_length
    )
    ...
    for seed in seeds:
        if all(c in char_to_idx for c in seed):
            generated = generate_text(
                network, chars, char_to_idx, idx_to_char, sequence_length, seed, 30
            )
